...
由于 $x\lor y=x\oplus y\oplus(x\land y)$，事实上此时已经可以求出任意两个 $a_i,a_j$ 的异或和，所以我们可以只考虑 $\land$。

下面假设你询问了 $a_i\land a_j$，下一次再询问，不可能是另外两个完全不同的位置。这是因为目前为止我们已知的所有信息都是对称的，对称的话就势必不能解出唯一解，除非你解出来的两个解是相等的，显然数据并不能保证这一点。

不妨设已知 $a_i\land a_j$ 与 $a_j\land a_k$，其中 $i,j,k$ 两两不同，接下来我们考虑某个二进制位的情况。

1. 若 $a_i\land a_j=1$ 或者 $a_j\land a_k=1$，那么 $a_j=1$。
2. 否则，若 $a_i\oplus a_j=0$ 或者 $a_j\oplus a_k=0$，那么 $a_j=0$。
3. 否则，此时我们可以推出 $\{a_i,a_j\}=\{a_j,a_k\}=\{0,1\}$，枚举不难发现只能是 $(a_i,a_j,a_k)=(0,1,0)$ 或 $(1,0,1)$。

我们会发现，最后的情况是无法确定出来到底是哪一种的。如果是 Easy Version 允许询问 $n+2$ 次，只要询问 $a_i\land a_k$ 就好了。
...
如果存在这样的情况，我们只需要问这两个位置的 $\land$ 或者 $\lor$，就可以求出这个位置的取值，然后解出每一个值了。
...
那么就必然存在一个 $a_k$ 使得 $a_1\oplus a_k=n-1$。这有什么特殊的？因为这两个数异或起来二进制全 $1$，那么它们不可能有两个位置都是 $1$，于是 $a_1\land a_k=0$，这就可以帮我们省掉一次询问。
