...
显然，这个题目需要数位 DP，记 $dp(u,k)$ 表示 ACAM 的节点 $u$ 走 $k$ 步合法的方案数，预处理出来。

设 $l=|n|$，那么显然长度小于 $l$ 的数是随便填的，对于这些情况，我们只需要枚举长度 $i$，然后枚举首位 $j=1\sim 9$（避免前导 $0$），累加 $dp(\text{trans}(0,j),i-1)$ 即可。

长度等于 $l$ 时，按照数位 DP 的套路，从高到底枚举，设前缀在 ACAM 上走到节点 $u$，那么下面的位置需要从 $u$ 开始走。

设当前枚举到 $i$ 位，那么枚举当前位填 $j=0\sim n_i-1$，注意，如果这是首位，$j$ 应当从 $1$ 开始枚举。然后累加 $dp(\text{trans}(u,j),l-i)$ 即可。
...
线段树合并时创建新的节点，可以做到保持子树中线段树的信息都正确，具体可以看[这篇文章](https://zhuanlan.zhihu.com/p/575513452)。
