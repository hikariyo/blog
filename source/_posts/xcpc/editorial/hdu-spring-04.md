...
根据这个思想，我们可以对每一个询问的 $r$ 二分找到这 $O(\log V)$ 个区间。具体地说，我们二分一个 $m$，然后用 ST 表查询 $m\sim i$ 的 $\gcd$ 即可，那么这样的话复杂度是单次 $O(\log n\log^2 V)$，其中二分是 $O(\log n)$、ST 表求一次 GCD 是 $O(\log V)$，区间数量是 $O(\log V)$。
...
对于后续的段，我们可以记录一下前面累积减去了多少，然后最大值用 ST 表查，于是单次询问复杂度 $O(\log n\log^2V)$。
